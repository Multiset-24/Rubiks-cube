//
// Created by sourav on 18-05-2025.
//
...
public:
    ...
    RubiksCube1dArray(const char initCube[6][3][3]) {
        for (int f = 0; f < 6; ++f) {
            for (int r = 0; r < 3; ++r) {
                for (int c = 0; c < 3; ++c) {
                    cube[getIndex(f, r, c)] = initCube[f][r][c];
                }
            }
        }
    }
    ...
    bool isSolved() const override {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 3; j++) {
                char c=cube[getIndex(i, j, 0)];
                for (int k = 0; k < 3; k++) {
                    if (this->cube[getIndex(i, j, k)] == c) continue;
                    else return false;
                }
            }
        }
